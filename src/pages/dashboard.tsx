...
import { getNavBars } from 'apiSdk/nav-bars';
import { NavBarInterface } from 'interfaces/nav-bar';
import { getParkings } from 'apiSdk/parkings';
import { ParkingInterface } from 'interfaces/parking';
import { getMenus } from 'apiSdk/menus';
import { MenuInterface } from 'interfaces/menu';

import { WidgetListPage as ShowCaseList } from 'pages/widgets';

function DashboardPage() {
  const { session } = useSession();

  const navBarStat = useSWR<PaginatedInterface<NavBarInterface>>(
    () => `/nav-bars`,
    () => getNavBars({ limit: 1, offset: 0 }),
  );

  const parkingStat = useSWR<PaginatedInterface<ParkingInterface>>(
    () => `/parkings`,
    () => getParkings({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Nav Bars',
      result: navBarStat?.data?.totalCount,
      isLoading: navBarStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Parkings',
      result: parkingStat?.data?.totalCount,
      isLoading: parkingStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Menus',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`widgets`} passHref legacyBehavior>
                  <Button
