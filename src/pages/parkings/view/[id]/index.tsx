...
import { getParkingById } from 'apiSdk/parkings';
...
function ParkingViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ParkingInterface>(
    () => (id ? `/parkings/${id}` : null),
    () =>
      getParkingById(id, {
        relations: ['user'],
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('parking', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/parkings/edit/${id}`} passHref legacyBehavior>
                    <Button
